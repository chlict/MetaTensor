...
TEST(TestTensorFormat, TestRowMajor) {
    ...
    auto off1 = AbstractLayoutProvider<RowMajorLayout>::offset(Dim2(1_c, 2_c), layout);
...
TEST(TestTensorFormat, TestColMajor) {
    ...
    auto layout = format.layout();
    auto off1 = AbstractLayoutProvider<ColMajorLayout>::offset(Dim2(1_c, 2_c), layout);
...
TEST(TestTensorFormat, TestOffset1) {
    ...
    hana::for_each(cp, [layout1](auto pair) {
        ...
        using LayoutProvider = typename format_traits<decltype(format1)>::layout_provider_type;
