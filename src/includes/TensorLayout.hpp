...
struct TensorLayout {
  ...
  constexpr TensorLayout(const Dimensions &dimensions, const Strides &stride)
      : dimensions_(dimensions), strides_(stride) {}

  // If 'Dimensions' is a 'Dims' type which consists of a hana::tuple,
  // layout.dimensions() cannot be used in constexpr but layout.dimensions_ can
  // (clang++ fixes this in clang11). However, we can use 'dimensions =
  // layout.dimensions()' outside of a constant evaluation context and use the
  // 'dimensions' in constant evaluation context.
  constexpr auto dimensions() const { return dimensions_; }
