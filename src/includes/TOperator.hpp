...
struct TOperator {
    static_assert(is_yap_expr_type<Expr> && is_toperand_type<Output> && all_are<toperand_tag, Inputs...>);
    ...
    constexpr TOperator(Expr const &expr, Output const &output, Inputs const &... inputs) :
...
//        static_assert(yap::get_arity(expr) == 2);
    ...
    constexpr auto gen_code_1_I_1_O() const {
        ...
        auto codes = [input1, output, expr]() {
            ...
            for (auto i1 = indices1.begin(), i2 = indices2.begin(); i1 != indices1.end() && i2 != indices2.end();
                i1++, i2++) {
                auto tile1 = input1.get_tile(*i1);
