...
struct TTiling {
    ...
    constexpr auto apply(Tensor &&tensor, Expr &&expr) const {
        ...
        // be careful not use size_t or unsigned for i or it will compile with error "not in the same ring" with hana data
...
constexpr auto Tiling2DRowMajor(RangeRow &&range_row, RangeCol &&range_col) {
    return TTiling(Dims(0_c, 1_c), std::forward<RangeRow>(range_row), std::forward<RangeCol>(range_col));
}

template <typename RangeRow, typename RangeCol>
constexpr auto Tiling2DColMajor(RangeRow &&range_row, RangeCol &&range_col) {
    return TTiling(Dims(1_c, 0_c), std::forward<RangeRow>(range_row), std::forward<RangeCol>(range_col));
}
