 ...
 * placeholders to newly allocated tensors. The second step is to replace the placeholders
 * with the tensors.
 */
...
constexpr auto deduce_tensor(Lhs &&lhs, ExprMap &&map) {
    ...
    auto result_tensor = Tensor(ElemType(), src_tensor.format(), Space(), src_tensor.addr());
    return result_tensor;
}

/*
 * This scanner does not change the ir list. It scans for placeholders and creates tensors.
 * During the scan, a map is established to record placeholder => tensor_expression.
 */
...
struct TempScanner {
    ...
    constexpr auto new_state(Expr &&expr) const {
        return hana::insert(map_, hana::make_pair(hana::llong_c<I>, std::move(expr)));
    }
...
//        return hana::insert(map_, hana::make_pair(hana::llong_c<I>, expr));
//        return expr;
