...
struct GenIRXform {
  ...
  constexpr GenIRXform(const Sequence &seq, const Stack &stk)
      : mIRList(seq), mStack(stk) {
    // printf("GenIR constructor:\n");
    // printf("mIRList: "); hana::for_each(mIRList, [](const auto &ir)
    // {yap::print(std::cout, ir);}); printf("mStack: "); hana::for_each(mStack,
    // [](const auto &expr) {yap::print(std::cout, expr);});
  }
  ...
  auto operator()(yap::expr_tag<Kind>, Expr1 &&lhs, Expr2 &&rhs) {
    ...
    auto genLhs = yap::transform(
        yap::as_expr(lhs),
        GenIRXform<decltype(mIRList), decltype(mStack), I>(mIRList, mStack));
    ...
    return GenIRXform<decltype(newIRList), decltype(newStack), index + 1>(
        newIRList, newStack);
  }

  template <typename Callable, typename... Args>
  auto operator()(yap::expr_tag<yap::expr_kind::call>, Callable &&callable,
                  Args &&...args) {
    // printf("GenIR: call matched\n");
    auto assign = yap::make_expression<yap::expr_kind::assign>(
        yap::make_terminal(temp_placeholder<I>{}),
        yap::make_expression<yap::expr_kind::call>(yap::as_expr(callable),
                                                   yap::as_expr(args)...));
    // printf("assign:\n"); yap::print(std::cout, assign);
    auto newIRList = hana::append(mIRList, std::move(assign));
    auto newStack = hana::append(
        mStack, std::move(yap::make_terminal(temp_placeholder<I>{})));
    return GenIRXform<decltype(newIRList), decltype(newStack), I + 1>{newIRList,
                                                                      newStack};
  }
};
...
struct GenIR : StaticTransform {
  ...
  constexpr auto transform(AST &&ast, Dumping dumping = DumpFlag::OFF{}) const {
    ...
    auto gen =
        yap::transform(std::forward<AST>(ast),
                       GenIRXform{hana::make_tuple(), hana::make_tuple()});
