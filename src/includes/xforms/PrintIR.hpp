...
std::ostream &print_type(std::ostream &os, hana::tuple<T> const &) {
  ...
  if (std::is_const<T>{}) os << " const";
  if (std::is_volatile<T>{}) os << " volatile";
  if (std::is_lvalue_reference<T>{}) os << " &";
  if (std::is_rvalue_reference<T>{}) os << " &&";
  return os;
}
...
struct print_impl {
  ...
  std::ostream &operator()(std::ostream &os, Expr const &expr, int indent,
    ...
    os << "\n";
    hana::for_each(
        expr.elements, [&os, indent, indent_str](auto const &element) {
