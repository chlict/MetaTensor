...
// Dim2(width, height) shape like a matrix, but the underlying layout has a Dim4
// shape (dimensions). LayoutProvider can be retrieved from format_traits
...
struct TensorFormat {
  ...
  constexpr TensorFormat(TensorFormat const& other) noexcept
      : shape_(other.shape_), layout_(other.layout_) {}

  constexpr TensorFormat(TensorFormat&& other) noexcept
      : shape_(std::move(other.shape_)), layout_(std::move(other.layout_)) {}

  constexpr auto shape() const { return shape_; }

  constexpr auto layout() const { return layout_; }

  constexpr auto dimensions() const { return layout_.dimensions(); }
...
template <typename Shape, typename LayoutProvider,
          typename =
              std::enable_if_t<is_a<layout_provider_tag, LayoutProvider>, void>>
constexpr auto make_format(
    Shape shape,
    AbstractLayoutProvider<LayoutProvider> const& layout_provider) {
...
template <typename Dim0, typename Dim1, typename LayoutProvider,
          typename = std::enable_if_t<
              is_integral_or_constant<Dim0> && is_integral_or_constant<Dim1> &&
